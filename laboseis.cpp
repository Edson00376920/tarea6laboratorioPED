...
void HojaCalc::sumarColumna(int columna){
   ...
   do{
       if(s->nCol == columna){
           ...
           cout <<"Suma de columnas "<<columna<<": "<< sum << endl;
           break;
       }
       s =s->sigCol;
   }while(s->nCol != 0);
...
int main(void)
{
    ...
    // la mariz se imprime por filas:
    hojaCalculo.imprimirHojaElec();
    hojaCalculo.sumarColumna(1);
    hojaCalculo.sumarColumna(2);
    hojaCalculo.sumarColumna(3);
    hojaCalculo.sumarColumna(4);
    hojaCalculo.sumarColumna(5);
    // me parece que se sumaban solo dos pero lo puse hasta 5
